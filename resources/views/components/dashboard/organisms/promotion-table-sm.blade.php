...
<div class="table_box table-thin">
    ...
    <form action="{{ route("admin.promotion.update") }}" method="post" class="filter-form pb-2" style="position: relative; z-index: 1;">
        @csrf
        @method('PATCH')
    ...
    <h1 class="text-title mb-1 mt-2 pt-2 border-top">Roll Back Students</h1>
    <form action="{{ route("admin.promotion.roll_back") }}" method="post" class="filter-form pb-1">
        @csrf
        @method('PATCH')
        <div class="input_group">
            <div class="input_field">
                <label for="section_id">From <span class="required">*</span></label>
                <select name="section_id" id="roll_back_section" required>
                    <option value="" selected disabled>Select Section</option>
                    @foreach ($sections->unique('class')->pluck('class') as $class)
                        <optgroup label="Class: {{ $class }}">
                            @foreach ($sections->where('class', $class) as $section)
            ...
            <button type="submit" class="btn mt-1" style="padding-block: 11px; margin-top: 26px;"><i class="fa-regular fa-arrow-down-wide-short"></i> Roll Back</button>
        </div>
    </form>
</div>